...
\*************************************************************************/
/*DigitalDCPSaoRecord.c*/
/* DigitalDCPSaoRecord.c - Record Support Routines for Digital DC PS */
/*
...
static long init_record(DigitalDCPSaoRecord *, int);
static long process(DigitalDCPSaoRecord *);
...
epicsExportAddress(rset,DigitalDCPSaoRSET);

struct DigitalDCPSaodset { /* DigitalDCPSao dset */
    long      number;
    DEVSUPFUN dev_report;
    DEVSUPFUN init;
    DEVSUPFUN init_record; /*returns: (-1,0)=>(failure,success)*/
    DEVSUPFUN get_ioint_info;
    DEVSUPFUN write_DigitalDC; /*returns: (-1,0)=>(failure,success)*/
	DEVSUPFUN special_linconv;
};

static void checkAlarms(DigitalDCPSaoRecord *);
static long fetch_value(DigitalDCPSaoRecord *, double *);
static void convert(DigitalDCPSaoRecord *, double);
static void monitor(DigitalDCPSaoRecord *);
static long writeValue(DigitalDCPSaoRecord *);

static long init_record(DigitalDCPSaoRecord *prec, int pass)
{
    struct DigitalDCPSaodset *pdset;
    ...
    if(!(pdset = (struct DigitalDCPSaodset *)(prec->dset))) {
	recGblRecordError(S_dev_noDSET,(void *)prec,"DigitalDCPSao: init_record");
	return(S_dev_noDSET);
    }
    ...
    /* must have write_DigitalDC function defined */
    if ((pdset->number < 6) || (pdset->write_DigitalDC ==NULL)) {
	recGblRecordError(S_dev_missingSup,(void *)prec,"DigitalDCPSao: init_record");
...
static long process(DigitalDCPSaoRecord *prec)
{
	struct DigitalDCPSaodset	*pdset = (struct DigitalDCPSaodset *)(prec->dset);
	long		 status=0;
	unsigned char    pact=prec->pact;
	double		value;

	if ((pdset==NULL) || (pdset->write_DigitalDC==NULL)) {
		prec->pact=TRUE;
		recGblRecordError(S_dev_missingSup,(void *)prec,"write_DigitalDC");
		return(S_dev_missingSup);
	}
...
{
    DigitalDCPSaoRecord     *prec = (DigitalDCPSaoRecord *)(paddr->precord);
    struct DigitalDCPSaodset       *pdset = (struct DigitalDCPSaodset *) (prec->dset);
...

static void checkAlarms(DigitalDCPSaoRecord *prec)
{
...

static long fetch_value(DigitalDCPSaoRecord *prec,double *pvalue)
{
	short		save_pact;
	long		status;
...

static void convert(DigitalDCPSaoRecord *prec, double value)
{
    /* check drive limits */
...

static void monitor(DigitalDCPSaoRecord *prec)
{
	...
	double		delta;
        ...
        if (monitor_mask){
                ...
                db_post_events(prec,&prec->val_v,monitor_mask);
                db_post_events(prec,&prec->state,monitor_mask);
                db_post_events(prec,&prec->psstat,monitor_mask);
	}
...

static long writeValue(DigitalDCPSaoRecord *prec)
{
	long		status;
        struct DigitalDCPSaodset 	*pdset = (struct DigitalDCPSaodset *) (prec->dset);

	if (prec->pact == TRUE){
		status=(*pdset->write_DigitalDC)(prec);
		return(status);
	}
